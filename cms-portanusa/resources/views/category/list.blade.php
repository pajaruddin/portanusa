        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-blue-madison sbold uppercase">Daftar Kategori</span>
                </div>
                <div class="actions">
                    <a class="btn green-haze btn-outline btn-circle btn-sm" href="/master-category/add-category">
                        Tambah Kategori
                    </a>
                </div>
            </div>
            <div class="portlet-body ">
                ...
                <table class="table table-striped table-bordered dt-responsive" id="tbl-category">
                    <thead>
                        <tr>
                            <th>No.</th>
                            <th>id</th>
...
$(function () {
    var table = $('#tbl-category').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{url('/'.'master-category'.'/data-category')}}",
        columns: [
    ...
    $('#tbl-category').on('switchChange.bootstrapSwitch', 'input[name="active"]', function (event, state) {
        var id = $(this).data('id');
        var active = "";
        if (state) {
            active = 'T';
        ...
        $.ajax({
            ...
            url: "{{url('/'.'master-category'.'/active-master-category')}}",
            data: {
                active: active,
                id: id
            },
            ...
            success: function (data) {
                if (data.status == "success") {
                    new Noty({
                        type: 'success',
                        text: 'Category berhasil di update',
                        layout: 'center',
                        timeout: 1000,
                        modal: true
                    }).show();
    ...
    $("#tbl-category").on("click", "#delete", function (event, state) {
        var id = $(this).data('id');
