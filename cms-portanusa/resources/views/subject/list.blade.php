        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-blue-madison sbold uppercase">Daftar Subject</span>
                </div>
                <div class="actions">
                    <a class="btn green-haze btn-outline btn-circle btn-sm" href="/master-subject/add-subject">
                        Tambah Subject
                    </a>
                </div>
            </div>
            <div class="portlet-body ">
                ...
                <table class="table table-striped table-bordered dt-responsive" id="tbl-subject">
                    <thead>
                        <tr>
                            <th>No.</th>
                            <th>id</th>
...
$(function () {
    var table = $('#tbl-subject').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{url('/'.'master-subject'.'/data-subject')}}",
        columns: [
    ...
    $('#tbl-subject').on('switchChange.bootstrapSwitch', 'input[name="active"]', function (event, state) {
        var id = $(this).data('id');
        var active = "";
        if (state) {
            active = 'T';
        ...
        $.ajax({
            ...
            url: "{{url('/'.'master-subject'.'/active-master-subject')}}",
            data: {
                active: active,
                id: id
            },
            ...
            success: function (data) {
                if (data.status == "success") {
                    new Noty({
                        type: 'success',
                        text: 'Subject berhasil di update',
                        layout: 'center',
                        timeout: 1000,
                        modal: true
                    }).show();
    ...
    $("#tbl-subject").on("click", "#delete", function (event, state) {
        var id = $(this).data('id');
