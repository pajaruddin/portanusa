    ...
    <div class="col-md-12">
        <div class="portlet light portlet-fit portlet-form">
            ...
            <div class="portlet-body ">
                <!-- BEGIN FORM-->
                <form method="POST" action="{{ url($menu."/".$submenu."/".$sale_event->id) }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data" role="form">
                    {{ csrf_field() }}
...
var counter_product = parseInt("{{$total_product_sale_event}}");
var product_array = [];
var data_product = <?= $sale_event_product_ids; ?>;

if (data_product.length > 0) {
    for (var i in data_product) {
    	product_array.push(data_product[i]);
    }
}
...
function formatRepo(repo) {
    ...
    var markup = "<div class='select2-result-repository clearfix'>" +
            ...
            "<div class='select2-result-repository__title'>" + repo.brand + "</div>";
...
function deleteProductItemRow(row_id) {
    var sale_event_product = $("#sale_event_product_" + row_id).val();
    if (sale_event_product != "") {
        var n = new Noty({
            text: 'Hapus produk dari sale event?',
            layout: 'center',
            modal: true,
            buttons: [
                Noty.button('YES', 'btn btn-success', function () {
                    $.ajax({
                        ...
                        url: "{{url('/'.$menu.'/delete-sale-event-product')}}",
                        data: {
                            id: sale_event_product
                        },
                        type: 'POST',
                        dataType: 'json',
